import { ArrowDownRight, ArrowUpRight, Boxes, CircleDollarSign, CreditCard, ShoppingBag, TrendingUp, Wallet } from 'lucide-react';
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-7">
        <KpiCard title="Bugungi savdo" value={money(todaySales)} icon={ShoppingBag} tone="blue" />
        <KpiCard title="Real pul kirimi" value={money(metrics.realIncome)} icon={ArrowUpRight} tone="green" />
        <KpiCard title="Chiqim" value={money(metrics.expense)} icon={ArrowDownRight} tone="red" />
        <KpiCard title="Sof foyda" value={money(metrics.profit)} icon={TrendingUp} tone={metrics.profit >= 0 ? 'green' : 'red'} />
        <KpiCard title="Mijozlar qarzi" value={money(metrics.debt)} icon={CircleDollarSign} tone="red" />
        <KpiCard title="Mening qarzlarim" value={money(metrics.payable)} icon={CreditCard} tone="amber" />
        <KpiCard title="Ombor qiymati" value={money(metrics.stock)} icon={Boxes} tone="amber" />
      </div>

      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        <div className="card p-5"><div className="mb-5"><h2 className="font-bold">Haftalik savdo</h2><p className="text-sm text-slate-500">So‘nggi 7 kunlik savdo dinamikasi</p></div><div className="h-72"><ResponsiveContainer><LineChart data={weekly}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" tickLine={false} axisLine={false} /><YAxis tickFormatter={(v) => `${Math.round(v / 1000000)} mln`} tickLine={false} axisLine={false} width={62} /><Tooltip formatter={(value) => money(Number(value))} /><Line type="monotone" dataKey="savdo" stroke="#2563eb" strokeWidth={3} dot={{ r: 4 }} /></LineChart></ResponsiveContainer></div></div>
