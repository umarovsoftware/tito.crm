...
export function SalesPage() {
  ...
  return <div>
    ...
    <div className="card mb-4 grid gap-3 p-4 md:grid-cols-[1fr_220px_240px]"><label className="relative self-start"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input className="input pl-10" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Sotuv ID, mahsulot yoki mijoz..." /></label><select className="input" value={payment} onChange={(event) => setPayment(event.target.value)}><option>Barchasi</option><option>Naqd</option><option>Karta</option><option>O‘tkazma</option><option>Qarz</option></select><select className="input" value={saleTypeFilter} onChange={(event) => setSaleTypeFilter(event.target.value as SaleTypeFilter)}><option>Barchasi</option><option>Doimiy mijoz</option><option>Tasodifiy xaridor</option></select></div>
