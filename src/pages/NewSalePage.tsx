...
import type { PaymentType, SaleItem, SaleType } from '../types';
import { createInitialSale, type SaleFormState } from '../utils/sale';
              ...
              {form.items.map((line, index) => {
                ...
                return (
                  <div key={`${line.perfumeId}-${index}`} className="grid gap-2 sm:grid-cols-[1fr_100px_130px_36px]">
                    <SearchSelect value={line.perfumeId} onChange={(perfumeId) => { const selected = data.perfumes.find((item) => item.id === perfumeId); changeItem(index, { perfumeId, sotuvNarxi: selected?.sotuvNarxi ?? 0 }); }} options={perfumeOptions} placeholder="Mahsulotni tanlang" />
                    <input className="input" type="number" onFocus={(e) => e.target.select()} min="1" value={line.miqdor} onChange={(event) => changeItem(index, { miqdor: Number(event.target.value) })} />
                    <input className="input" type="number" onFocus={(e) => e.target.select()} min="1" value={line.sotuvNarxi} onChange={(event) => changeItem(index, { sotuvNarxi: Number(event.target.value) })} />
                    ...
                    {product && line.miqdor > product.qoldiq && <p className="text-xs text-red-600 sm:col-span-4">Qoldiq: {product.qoldiq} dona</p>}
                  </div>
                );
              })}
