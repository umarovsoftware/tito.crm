...
const initial = { username: '', ism: '', familiya: '', email: '', faol: true, parol: '' };
...
export function EmployeesPage() {
  ...
  const edit = (item: Employee) => { setForm({ id: item.id, username: item.username, ism: item.ism, familiya: item.familiya, email: item.email, faol: item.faol, parol: '' }); setOpen(true); };
      ...
      <div className="card overflow-hidden">
        ...
        <div className="divide-y lg:hidden">
          {rows.map((item) => (
            <div key={item.id} className="cursor-pointer p-4" onClick={() => navigate(`/hodimlar/${item.id}`)}>
              ...
              <p className="mt-2 text-xs text-slate-500">{item.email || 'Email kiritilmagan'} · Qo‘shilgan: {formatDate(item.createdAt)}</p>
        ...
        <div className="hidden overflow-x-auto lg:block">
          <table className="w-full text-sm">
            <thead className="table-head"><tr><th className="px-4 py-3">Hodim</th><th className="px-4 py-3">Email</th><th className="px-4 py-3">Holat</th><th className="px-4 py-3">Qo‘shilgan sana</th><th className="px-4 py-3 text-right">Amallar</th></tr></thead>
