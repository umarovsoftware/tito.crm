...
export function FinanceCrudPage({ mode }: { mode: 'income' | 'expense' }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (form.summa <= 0 || !form.sana) return showToast('Summa va sanani to‘g‘ri kiriting.', 'warning');
    setSaving(true);
  ...
  return <div><PageHeader title={isIncome ? 'Kirimlar' : 'Chiqimlar'} description={isIncome ? 'Real pul kirimlari.' : 'Do‘kon xarajatlari.'} actions={<button className="btn-primary" onClick={() => { setForm({ kategoriya: 'Boshqa', summa: 0, sana: today(), izoh: '' }); setOpen(true); }}><Plus size={18} /> {isIncome ? 'Kirim' : 'Chiqim'} qo‘shish</button>} />
