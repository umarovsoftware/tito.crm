...
import { createInitialSale, getGuestCode, getSaleCode, saleTotal, type SaleFormState } from '../utils/sale';
...
export function SalesPage() {
  ...
  const handleScanned = (code: string) => {
    ...
    setForm((current) => {
      ...
      return { ...current, items: current.items.map((item, index) => index === existingIndex ? { ...item, miqdor: item.miqdor + 1 } : item) };
    });
